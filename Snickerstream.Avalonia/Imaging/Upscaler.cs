...
// ScaleFX/MMPX) → neural (the Anime4K CNN sizes, S through VL) → MetalFx (Apple).
...
// `Fsr`/`Anime4K` are the simple Lanczos stand-ins (shown as "Lanczos"/"Lanczos+"); `Fsr1` (appended after
// MetalFx so existing mac settings keep their index) is the real FidelityFX FSR 1 algorithm (shown as
// "FSR", multi-pass GPU — see Views/Fsr.cs).
public enum UpscaleFilter { None, Sharp, Fsr, Anime4K, Xbr, SuperXbr, ScaleFx, Mmpx, Anime4KCnn, Anime4KCnnM, Anime4KCnnL, Anime4KCnnVL, MetalFx, Fsr1 }
